...
const hydrateOrg = (record: OrgRecord): Org => ({
  ...record,
...
const hydrateClaim = (record: ClaimRecord): Claim => ({
  ...record,
  evidence: EVIDENCE.filter(item => item.claimId === record.id).map(item => ({ ...item }))
});

const matchesFilter = (record: OrgRecord, q?: string, filters?: Record<string, unknown>): boolean => {
...
export default class MockProvider implements IDataProvider {
  async searchEntities(q?: string, filters?: Record<string, unknown>): Promise<Org[]> {
    return ORGS.filter(record => matchesFilter(record, q, filters)).map(record => hydrateOrg(record));
  }

  async getEntityBySlug(slug: string): Promise<Org | undefined> {
    let match: OrgRecord | undefined;
  ...
  async getProjects(orgId: Id): Promise<Project[]> {
  ...
  async getClaims(orgId: Id): Promise<Claim[]> {
    return CLAIMS.filter(claim => claim.orgId === orgId).map(claim => hydrateClaim(claim));
  }

  async getRelations(entityId: Id, opts?: { since?: string; types?: RelationType[] }): Promise<Relation[]> {
  ...
  async getDocuments(orgId: Id): Promise<Document[]> {
    return DOCUMENTS.filter(document => document.orgId === orgId).map(document => ({ ...document }));
  }

  async getSignals(filters?: Record<string, unknown>): Promise<Signal[]> {
    const entityId = typeof filters?.entityId === 'string' ? (filters.entityId as Id) : undefined;
    const rag = typeof filters?.rag === 'string' ? (filters.rag as RAG) : undefined;
  ...
  async proposeWrite(kind: 'relation' | 'claim' | 'dossier_text', payload: unknown): Promise<{ draftId: Id }> {
    const draft: Draft = { id: `draft-${Date.now().toString(36)}`, kind, payload, created_on: todayIso(), state: 'proposed' };
    drafts = [...drafts, draft];
    return { draftId: draft.id };
  }

  async approveWrite(draftId: Id): Promise<void> {
    drafts = drafts.filter(draft => draft.id !== draftId);
  }

  async askAI(query: string, ctx?: AskAiContext): Promise<{ answer: string; citations: { title: string; url: string }[] }> {
  ...
  async validateDossier(json: unknown): Promise<{ ok: boolean; errors: string[]; warnings: string[]; patched?: unknown }> {
